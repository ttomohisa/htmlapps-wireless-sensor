...
最大4台のスマートフォンをワイヤレスな動きセンサーとしてPCへ同時接続し、加速度・回転・傾きをWebRTCで直接送信して可視化し、端末間の時計差を補正した共通時間軸で記録する、プライバシー重視の Browser Kitty 単一HTMLツールです。
...
- **個別表示と重ね合わせ** — 接続中センサー一覧から1台を選んで現在値・3D表示・RTTを確認でき、グラフは「選択中」と「全センサー重ね合わせ」を切り替えられます。
...
9. グラフは **選択中** / **全センサー重ね合わせ** を切り替え、5 / 10 / 30 / 60秒の範囲で確認できます。**選択中の姿勢を0にする** は端末ごとに独立して適用されます。
10. 接続直後に時計同期を自動調整し、その後も定期的に再同期します。ヘッダーとセンサーカードの **同期** 表示で推定誤差を確認できます。
11. 記録を開始すると、補正済みの共通時間軸で全センサーを記録します。停止後、指定したファイル名でCSVまたはJSONを保存します。
...
- v0.11.0では同時接続は最大4台です。GPS、磁気、マイク計測、QR接続以外のカメラ計測は対象外です。
