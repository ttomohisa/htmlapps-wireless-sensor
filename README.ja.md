...
- **両方向ともカメラQRが基本** — 受信側を選ぶとPCのOffer QRを自動生成し、スマホ側を選ぶとQRカメラを自動起動。続いてスマホの分割Answer QRをPCカメラで読み取ります。接続確認中・再接続中・失敗時の案内に加え、ICE収集状態、ローカル/相手候補の種類、候補ペア成立状況も表示します。
...
7. 送信頻度は **省電力 / 標準 / 高精度** から選べます。これは送信の間引き設定であり、端末のセンサー取得頻度を保証するものではありません。省電力ではスマホ画面の数値更新も減らします。
8. **測定中は画面をオンに保つ** をOFFにするとバッテリーを節約できます。ただし画面消灯後にブラウザーがセンサーイベントを止める端末があるため、連続計測ではONを推奨します。
...
- 接続QR / 返答QRはICE候補収集完了後にだけ生成します。候補収集が15秒以内に完了しない場合はその試行を破棄し、新しいPeerConnectionで再試行します。接続失敗時は画面のICE診断（候補数・候補種別・ICE状態・候補ペア）も確認してください。
